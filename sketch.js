...
function setup() {
	...
	if (bgImgLoaded) {
		...
		console.warn("Background image not loaded, skipping resize");
	  }
	allCactiImgs = [
...
function draw() {
	...
	text(scoreText, width - scoreWidth - 100, 100);
  	...
  	// Add this line to draw the question mark and controls
  	drawQuestionMarkAndControls();
...
function drawQuestionMarkAndControls() {
  // Draw question mark
  textFont(font);
  textSize(24);
  fill(53);
  ...
  // Check if mouse is over the question mark
  ...
  // Draw controls if hovering over question mark
  if (showControls) {
    textSize(16);
    let yPos = 60;
    let xPos = 20;
    ...
    text("1-5: Change speed (1x, 2x, 5x, 10x, 30x)", xPos, yPos);
    yPos += 20;
    text("N: Toggle neural network visualization", xPos, yPos);
    yPos += 20;
    text("B: Toggle closest obstacle visualization", xPos, yPos);
    yPos += 20;
    text("V: Toggle hitboxes visualization", xPos, yPos);
  }
}
